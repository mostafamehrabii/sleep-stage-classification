...
# Option 1: Weighted sampler only (recommended for severe imbalance)
USE_CLASS_WEIGHTS = False
USE_WEIGHTED_SAMPLER = True

# Fixed early stopping: 'val_loss', 'f1_macro', or 'hybrid'
...
class ResNet1D(nn.Module):
    ...
    def _make_layer(self, out_ch, blocks, kernel_size=7, stride=1):
        ...
        layers = []
        layers.append(ResidualBlock1D(self.in_channels, out_ch, kernel_size, stride, downsample))
        self.in_channels = out_ch
        for _ in range(1, blocks):
            layers.append(ResidualBlock1D(out_ch, out_ch, kernel_size))
...
def get_model(model_type='resnet8', in_ch=2, num_classes=4):
    configs = {'resnet8':[2,2,2,2], 'resnet12':[3,3,3,3], 'resnet16':[4,4,4,4]}
    return ResNet1D(in_ch=in_ch, num_classes=num_classes, blocks=configs[model_type])
...
def train_and_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, fold_idx, model_type='resnet8'):
    ...
    for epoch in pbar:
        ...
        # FIXED: Corrected early stopping logic
        improved = False
        if EARLY_STOP_MODE == 'val_loss':
            # Mode 1: Only loss matters
            if val_loss < best_val_loss - 1e-8:
                improved = True
                best_val_loss = val_loss
        elif EARLY_STOP_MODE == 'f1_macro':
            # Mode 2: Only F1 matters
            if val_f1 > best_val_f1 + 1e-8:
                improved = True
                best_val_f1 = val_f1
        else:  # hybrid mode
            # Mode 3: F1 is primary, loss is tie-breaker
            # Improve if: (1) F1 increases, OR (2) F1 stays same but loss decreases
            if val_f1 > best_val_f1 + 1e-8:
                # F1 improved - always save
                improved = True
                best_val_f1 = val_f1
                best_val_loss = val_loss
            elif abs(val_f1 - best_val_f1) < 1e-6 and val_loss < best_val_loss - 1e-8:
                # F1 essentially same but loss improved - save
                improved = True
                best_val_loss = val_loss
...
def main():
    t0 = time.time()
    print("="*80)
    print("SLEEP STAGE CLASSIFICATION - OPTIMIZED FOR A40 (48GB VRAM)")
    print("="*80)
    print_gpu_memory()
